...
def get_info_from_csv(path):
    ...
    # 5 slices, # trials, 3 vector components
    slices = np.ndarray(shape=(5, num_trials, 3))
...
def get_heatmap_and_indices(slices, save_figures):
    ...
    for i in range(5):
        ...
        hist, _, _ = np.histogram2d(
            slices[i][:, 1], slices[i][:, 0], bins=[lon_edges, lat_edges])
...
def calculate_bucket_sds(heatmap_indices, heatmaps, slices):
    ...
    for s in range(5):
        ...
        # at that slice
        std_deviations_slice = np.full(
            shape=(len(heatmaps[s]), len(heatmaps[s][0])), fill_value=None)
        ...
        for i in range(len(heatmap_indices[s])):
            ...
            if std_deviations_slice[heatmap_index[0]-1, heatmap_index[1]-1] is None:
                std_deviations_slice[heatmap_index[0]-1,
                                     heatmap_index[1]-1] = [depth_at_index]
            else:
                std_deviations_slice[heatmap_index[0]-1,
                                     heatmap_index[1]-1].append(depth_at_index)
        for i in range(len(std_deviations_slice)):
            for j in range(len(std_deviations_slice[0])):
                if std_deviations_slice[i][j] is not None:
                    std_deviations_slice[i][j] = np.std(
                        std_deviations_slice[i][j])
...
def run_search_algorithm(indices_list, std_deviations_list, visited_list, probabilities_list):
    ...
    for i in range(5):
        ...
        histogram = []
        start_node_index = np.where(
            probabilities_list[i] == np.max(probabilities_list[i]))[0][0]
        current_node_index = start_node_index
        num_nodes_visited = 1
        visited_list[i][current_node_index] = True
        time_remaining -= SEARCH_TIME
        histogram.append([(indices_list[i][start_node_index][0], indices_list[i][start_node_index][1]), probabilities_list[i]
                         [start_node_index], max(0.7 * DEPTH_FACTOR - 0.007 * std_deviations_list[i][start_node_index], 0.2), time_remaining])
        # Until we're done:
        while num_nodes_visited < len(indices_list[i]) and time_remaining > 0:
            ...
            for neighbor in range(len(indices_list[i])):
                ...
                travel_distance = eudis5(
                    indices_list[i][current_node_index], indices_list[i][neighbor]) * METERS_PER_BIN
                ...
                p_we_find_it = max(0.70 * DEPTH_FACTOR - 0.007 *
                                   std_deviations_list[i][neighbor], 0.2)

                # Value ratio
                value_ratio = (p_submarine_is_here *
                               p_we_find_it) / travel_time
            ...
            num_nodes_visited += 1
            histogram.append([(indices_list[i][best_candidate_index][0], indices_list[i][best_candidate_index]
                             [1]), best_candidate_p_it_is_here, best_candidate_p_we_find_it, time_remaining])
...
def plot_search_path(histograms, i, directory):
    ...
    for index in range(len(histograms[i])):
        coordinates[index][0] = lat_edge_array[i][histograms[i]
                                                  [index][0][0]] + .0025
        coordinates[index][1] = lon_edge_array[i][histograms[i]
                                                  [index][0][1]] + .0025
    # Create a heatmap for the probability array
    heatmap = plt.imshow(heatmaps[i], cmap='binary', origin='lower', extent=(min(lon_edge_array[i]), max(
        lon_edge_array[i]), min(lat_edge_array[i]), max(lat_edge_array[i])), alpha=0.8, vmin=0.00001)

    # Overlay scatter plot with lines connecting successive points
    plt.scatter(coordinates[:, 1], coordinates[:, 0],
                color='black', marker='', label='Coordinates', linewidths=1)
    print(len(coordinates))
    # Connect successive points with lines
    for index in range(1, len(coordinates)):
        plt.plot([coordinates[index-1][1], coordinates[index][1]], [coordinates[index-1]
                 [0], coordinates[index][0]], color='black', label='_nolegend_')
...
heatmaps, heatmap_indices, lat_edge_array, lon_edge_array = get_heatmap_and_indices(
    slices, True)
...
for i in range(5):
    ...
    for j in range(len(indices_list[i])):
        std_devs[j] = std_deviations[i][indices_list[i]
                                        [j][0]][indices_list[i][j][1]]
...
for i in range(5):
    ...
    for j in range(len(indices_list[i])):
        probabilities[j] = heatmaps[i][indices_list[i]
                                       [j][0]][indices_list[i][j][1]]
    probabilities_list.append(probabilities)


histograms = run_search_algorithm(
    indices_list, std_deviations_list, visited_list, probabilities_list)
# print(histograms[3])
...
plt.plot(hours_plot, aggregate_probability_of_find,
