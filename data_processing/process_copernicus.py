...
for i in range(len(latitude_array)):
    ...
    for j in range(len(longitude_array)):
        latitude_line.append(np.linalg.norm(
            [eastward_slice[i][j], northward_slice[i][j], vertical_slice[i][j]]))
...
plt.quiver(lon, lat, eastward_component[39],
           northward_component[39],  color='blue', width=0.002)
