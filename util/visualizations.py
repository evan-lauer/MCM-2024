...
def plot_ocean_floor(lat_start, lat_end, long_start, long_end):
    ...
    lat_lower_bound = get_nearest_index_hi_or_lo(
        lat_start, latitude_array, smaller=True)
    lat_upper_bound = get_nearest_index_hi_or_lo(
        lat_end, latitude_array, smaller=False)
    long_lower_bound = get_nearest_index_hi_or_lo(
        long_start, longitude_array, smaller=True)
    long_upper_bound = get_nearest_index_hi_or_lo(
        long_end, longitude_array, smaller=False)

    elevation_array = dataset.variables['elevation'][lat_lower_bound:lat_upper_bound,
                                                     long_lower_bound:long_upper_bound]

    lon, lat = np.meshgrid(
        longitude_array[long_lower_bound:long_upper_bound], latitude_array[lat_lower_bound:lat_upper_bound])
...
def plot_current_at_depth(depth, lat_start, lat_end, long_start, long_end, directory):
    ...
    eastward_component = horiz_dataset.variables['uo'][:, :,
                                                       lat_lower_bound:lat_upper_bound, long_lower_bound:long_upper_bound][0][depth_index]
    northward_component = horiz_dataset.variables['vo'][:, :,
                                                        lat_lower_bound:lat_upper_bound, long_lower_bound:long_upper_bound][0][depth_index]
    ...
    plt.quiver(lon, lat, eastward_component,
               northward_component,  color='blue', width=0.002)
    plt.title('Horizontal Currents at Depth = ' +
              str(depth_array[depth_index]) + 'm')
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    # plt.show()
    plt.savefig('./trial_data/' + directory +
                '/current_slice.png', bbox_inches='tight')
