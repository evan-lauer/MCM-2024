...
horiz_dataset = nc.Dataset(
    './oceanographic_data/Copernicus_currents/' + horiz_currents_filename)
vert_dataset = nc.Dataset(
    './oceanographic_data/Copernicus_currents/' + vert_currents_filename)
...
# We need to scale up our m/s velocities by the number of steps
SCALE_FACTOR = STEP_SIZE
# Duration: seconds (make sure DURATION is divisible by STEP_SIZE)
DURATION = 60 * 60 * 24 * 5
...
STARTING_LATITUDES = np.random.uniform(
    initial_coordinates[0] - lat_long_variation, initial_coordinates[0] + lat_long_variation, size=NUM_TRIALS)
STARTING_LONGITUDES = np.random.uniform(
    initial_coordinates[1] - lat_long_variation, initial_coordinates[1] + lat_long_variation, size=NUM_TRIALS)
# Generate a series of random starting depths, varying uniformly from += depth_variation
STARTING_DEPTHS = np.random.uniform(
    initial_coordinates[2] - depth_variation, initial_coordinates[2] + depth_variation, size=NUM_TRIALS)
...
#
# for each submersible, there is a xyz component of displacement. Format: (m_north, m_east, m_vert)
displacement_vectors = np.ndarray(
    shape=(NUM_STEPS, NUM_TRIALS, 3), dtype=np.float64)
displacement_vectors.fill(0)
# for each time step, for each submersible, there is a lat and long. This is for graphing later
coordinate_vectors = np.ndarray(shape=(NUM_STEPS, NUM_TRIALS, 3))
...
for s in range(NUM_STEPS):
    ...
    for t in range(NUM_TRIALS):
        ...
        latitude, longitude = add_meters_north(
            STARTING_LATITUDES[t], STARTING_LONGITUDES[t], displacement_vectors[s][t][0])
        latitude, longitude = add_meters_east(
            latitude, longitude, displacement_vectors[s][t][1])
        ...
        i_lat = get_nearest_index_closest_val(
            latitude + OFFSETS[t], latitude_array)
        i_long = get_nearest_index_closest_val(
            longitude + OFFSETS[t], longitude_array)
        i_depth = get_nearest_index_closest_val(
            depth + OFFSETS[t], depth_array)
            ...
            if is_valid_ocean(latitude, longitude, depth) and s > 0:
                ...
                if s == NUM_STEPS - 1:
                    ...
                    displacement_vectors[s+1][t] = displacement_vectors[s][t] + \
                        (displacement_vectors[s][t] -
                         displacement_vectors[s-1][t])
                continue
            else:
                # If we aren't, then beach us
                is_beached[t] = True
                for remaining in range(s, NUM_STEPS):
                    coordinate_vectors[remaining][t] = np.array(
        ...
        r_v_north, r_v_east, r_v_vertical = compute_random_vector(np.linalg.norm(
            np.array([v_north, v_east, v_vertical])), gaussian_sample_step[t])

        # print([v_north,v_east,v_vertical])
        # print([r_v_north,r_v_east,r_v_vertical])

        # Make sure depth cannot be negative
        new_displacement = displacement_vectors[s][t] + SCALE_FACTOR * np.array(
            [v_north, v_east, v_vertical]) + SCALE_FACTOR * np.array([r_v_north, r_v_east, r_v_vertical])
...
def plot_from_above(coordinate_vectors):
    ...
    for i in range(NUM_TRIALS):
        ...
        ax.plot(longitudes, latitudes, marker='o', markersize=1,
                linewidth=1, label=f'Simulation {i+1}')
...
def plot_in_3d(coordinate_vectors, directory):
    ...
    for i in range(NUM_TRIALS):
        ...
        ax.scatter(longitudes[0], latitudes[0], depths[0],
                   c='green', marker='o')  # Start point
        ax.scatter(longitudes[-1], latitudes[-1],
                   depths[-1], c='red', marker='x')  # End point
    ...
    if SAVE_PLOTS and NUM_TRIALS == 30:
        ...
        plt.savefig('./trial_data/' + directory +
                    '/frontview.png', bbox_inches='tight')
        ax.view_init(elev=5, azim=0)
        plt.savefig('./trial_data/' + directory +
                    '/leftview.png', bbox_inches='tight')
        ax.view_init(elev=45, azim=45)
        plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
        plt.savefig('./trial_data/' + directory + '/cornerview.png')
        ax.view_init(elev=90, azim=90)
        plt.savefig('./trial_data/' + directory +
                    '/topview.png', bbox_inches='tight')
...
if SAVE_PLOTS and NUM_TRIALS == 200:
    write_trial_data_to_csv(coordinate_vectors, NUM_TRIALS,
                            NUM_STEPS, initial_coordinates, directory)
plot_in_3d(coordinate_vectors, directory)
if SAVE_PLOTS:
    plot_current_at_depth(
        initial_coordinates[2], 37, 40, 17.5, 21.5, directory)
